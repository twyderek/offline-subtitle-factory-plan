...
- `c3e7b7da6b0f62dec52f9d65da4d60294e7f3bad` 的 tracked-clean worktree 已通過 `npm ci`、完整 `npm run check`、Breeze x64／arm64 focused regression、release metadata contract、`npm run docs:check` 與 `git diff --check`；已建立 macOS arm64 acceptance-only DMG／ZIP 並通過 archive、DMG verify、ad-hoc codesign、embedded notes、`latest-mac.yml` version／naming 與 final package renderer smoke 核對。renderer smoke 包含 settings、Breeze selection/cancel、manual job completion、trim、AI review/provider 與 folder flow。
- Windows CI、Windows local／pristine lifecycle 與公開 per-asset provenance 仍是外部 gate，尚未執行／驗證；本輪不推送、不建立公開 `v0.51.1`，因此 patch preparation 維持 `NOT_READY_EXTERNAL_GATES_REQUIRED`，未完成前不得稱為可公開發布。完整 evidence：`docs/project-management/evidence/2026-09-02-patch-preparation-0511.json`。
