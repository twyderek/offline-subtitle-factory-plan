...
- `75b24daaac9a00536e402565cadbe031200deef2` 的 tracked-clean worktree 已通過 `npm ci`、完整 `npm run check`、Breeze x64／arm64 focused regression、release metadata contract、`npm run docs:check` 與 `git diff --check`；已建立 macOS arm64 acceptance-only DMG／ZIP 並通過 archive、DMG verify、ad-hoc codesign、embedded notes 與 `latest-mac.yml` version／naming 核對。packaged renderer smoke 在本機 host 逾時未找到 page target，標示 `TEST_GAP`，不宣稱通過。
