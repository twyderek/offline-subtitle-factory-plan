...
- 發布授權：
  ...
  - 核准範圍：需求方已同意打包、提交、推送、更新既有 release PR #2，並接受以未簽章、未公證、未實機測試與 `CI=PENDING` 作為 release preparation 風險揭露；不授權建立 `v0.50.1` tag、GitHub Release 或公開安裝資產
- 遺留風險與後續事項：`LIVE_PROVIDER_TESTS=NOT_RUN`；真實 Ollama／LM Studio、取消／timeout／checkpoint 端到端、GitHub Actions（`CI=PENDING`）、Windows clean-machine、Authenticode、macOS notarization 與正式 0.50.1 asset 仍未驗收或建立。新 recheck installer 已含 `app-update.yml`；unsigned／未公證限制仍須在後續公開發布前重新核對。下一步等待 Actions run 後更新 CI 證據，再由需求方審查並人工合併既有 PR #2；公開 tag／Release 需另行授權。
